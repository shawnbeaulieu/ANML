...
class Learner(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.memory_module = nn.LSTM(64,64)
        ...
        for i, (name, param) in enumerate(self.config):
            if 'conv' in name:
                ...
                for name in self.memory_module.named_parameters():
                #if 'weight' in name[0]:
                    self.vars.append(nn.Parameter(name[1]))
    ...
    def forward(self, x, hidden_state, vars=None, bn_training=True, feature=False):
        """
        ...
        if self.neuromodulation:
            ...
            for i in range(x.size(0)):
                ...
                data_ = data_.view(data.size(0), 64)

                #lstm_weights = [vars[12], vars[13], vars[14], vars[15]]
                #for name in self.memory_module.named_parameters():
                
                    #if name[0] == 'weight_ih_l0':
                    #    name[1].data = vars[12]

                    #elif name[0] == 'weight_hh_l0':
                    #    name[1].data = vars[13]

                    #elif name[0] == 'bias_ih_l0':
                    #    name[1].data = vars[14]

                    #elif name[0] == 'bias_hh_l0':
                    #    name[1].data = vars[15]

                lstm_input = data_.view(1,1,-1)

                data_, hidden_state = self.memory_module(lstm_input, hidden_state)

                data_.view(data.size(0), 64)

                w,b = vars[12], vars[13]
                conv1_mask = F.relu(F.linear(data_, w, b)).view(64,3,3,3)

                w,b = vars[14], vars[15]
                conv2_mask = F.relu(F.linear(data_, w, b)).view(64,64,3,3)

                w,b = vars[16], vars[17]
                conv3_mask = F.relu(F.linear(data_, w, b)).view(64,64,3,3)
